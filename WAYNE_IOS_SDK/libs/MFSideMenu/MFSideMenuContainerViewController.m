...
- (void)setCenterViewController:(UIViewController *)centerViewController {
    ...
    [self.centerViewController view].layer.anchorPoint =CGPointMake(1.0,0.5);
...
- (UITapGestureRecognizer *)centerTapGestureRecognizer{
    UITapGestureRecognizer *tapRecognizer = [[UITapGestureRecognizer alloc]initWithTarget:self action:@selector(centerViewControllerTapped:)];
...
- (void)sendStateEventNotification:(MFSideMenuStateEvent)event {
    ...
    NSString *eventStr=[NSString stringWithFormat:@"%u",event];
    NSDictionary *userInfo = [NSDictionary dictionaryWithObject:eventStr  forKey:@"eventType"];
...
- (void)setLeftSideMenuFrameToClosedPosition {
    ...
    leftFrame.size.width =[UIScreen mainScreen].bounds.size.width;
...
- (void) setCenterViewControllerOffset:(CGFloat)xOffset {
    NSLog(@"%f%f",xOffset,self.leftMenuWidth);
    float maxScale = 0.85;
    float scale = 1-((1-maxScale)*(xOffset/self.leftMenuWidth));
    NSLog(@"====%f",scale);
    
//    [self.centerViewController view].transform = CGAffineTransformMakeTranslation(xOffset*scale, 0);
//    [self.centerViewController view].transform = CGAffineTransformScale([self.centerViewController view].transform, scale, scale);
    
    [self.centerViewController view].transform = CGAffineTransformMakeScale(scale, scale);
    [self.centerViewController view].transform = CGAffineTransformTranslate([self.centerViewController view].transform, xOffset, 0);
    
//    [self.centerViewController view].transform = CGAffineTransformMakeScale(scale, scale);
//    CGRect frame = [self.centerViewController view].frame;
//    frame.origin.x = xOffset*(scale);
//    [self.centerViewController view].frame = frame;
    
    
//    [self.centerViewController view].transform = CGAffineTransformMakeTranslation(xOffset, 0);
